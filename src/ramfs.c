...
int ramfs_setup_mount(struct filesystem *fs, struct mount *mnt)
{
    ...
    char *fptr = (char *)INITRD_BASE;
    int idx = 0;
    while (memcmp(fptr + sizeof(struct cpio_t), "TRAILER!!!", 10)) {
        struct cpio_t *header = (struct cpio_t *)fptr;
        ...
        strncpy(inode->name, fptr + sizeof(struct cpio_t), namesize);
